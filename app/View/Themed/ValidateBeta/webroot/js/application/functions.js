...
jQuery("[data-toggle=popover-info]").popover({template:'<div class="popover" role="tooltip"><div class="arrow"></div><h3 class="popover-title info"></h3><div class="popover-content"></div></div>'});
...
//Page 2 - Application :: start
	if(jQuery('#basicWizard').length>0){
		...
		//Ignore hidden fields (tabs that are not displayed)
	...
	function sendValidationCode(){
		...
		jQuery.ajax({
			url: 'http://stage.peerkeyloan.com/application/sendValidationCode',
			method:'post',
			data:sender,
			headers:{
	...
	function verifyValidationCode(){
		jQuery.ajax({
			url: 'http://stage.peerkeyloan.com/application/verifyValidationCode/'+jQuery('#ValidateEmail').val(),
			method:'POST',
			async:false,
			headers:{
				'x-keyStone-nonce': nonce
