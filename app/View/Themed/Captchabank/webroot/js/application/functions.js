...
//Page 2 - Application :: start
	...
	jQuery('#captcha-container').on('click', 'li.captcha-icon', function(){
		jQuery('#visual-captchares').val(jQuery(this).find("span").attr('id'));
		jQuery('.captcha-icon').each(function(){
			jQuery(this).css('background-color','transparent');
		});
				
		jQuery(this).css('background-color','#8dc63f');
		validateCaptcha();
	});
	
	function validateCaptcha(){
		var data = {'request':jQuery('#visual-captchareq').val(),'chosen':jQuery('#visual-captchares').val()}
		
		jQuery.ajax({
			url: '/application/validateCaptcha',
			data: data,
			method:'post',
			headers:{
			...
			complete: function(xhr, str){
				...
				if(status == 'success'){
					jQuery('.btnnext').fadeIn();
				}else{
					jQuery('.btnnext').fadeOut();
					jQuery('#captcha-container').html();
					regenerateCaptcha();
				}
	...
	function regenerateCaptcha(){
		jQuery.ajax({
			url: '/application/regenerateCaptcha',
			method:'get',
			headers:{
				'x-keyStone-nonce': nonce
			},
			complete: function(xhr, str){
				var htmlblock = xhr.responseJSON.data;
				jQuery('#captcha-container').html(htmlblock);
			}
		});
	}
